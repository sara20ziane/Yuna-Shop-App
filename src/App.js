...
const SUGGESTED_COMMUNES = {
  "16 Alger": [
    ...
    "El Biar", "Bachidjerrah", "Dar El Beïda", "Bordj El Kiffan", "Rouiba",
    "Reghaïa", "Ain Taya",
  ],
  "09 Blida": [
...
const MainApp = ({ user }) => {
  ...
  const defaultConfig = {
    categories: [
      "Mode Femme", "Beauté", "Chaussures", "Kids", "Maison", "Accessoires",
    ],
    sizes: ["XS", "S", "M", "L", "XL", "2 ans", "4 ans", "Unique"],
    colors: [
      "Beige Nude", "Rose Poudré", "Blanc Cassé", "Taupe", "Marron Clair", "Noir",
    ],
  };
  ...
  const calculateTotals = (items, shipNat, orderDate) => {
    ...
    const processed = items?.map((it) => {
      ...
      const itLogInt = (effectiveWeight / 1000) * stats.rate;

      venteTotal += effectiveVente;
      ...
      return {
        ...
        itAchatDA,
        itLogInt,
        itBenefit: effectiveVente - (itAchatDA + itLogInt),
      };
    }) || [];
  ...
  const getCalculatedWeightForArrivage = (arrivageId) => {
    ...
    orders.forEach((o) => {
      (o.items || []).forEach((item) => {
        if (item.arrivageId === arrivageId && item.status !== "Annulé")
  ...
  const lateDeliveries = useMemo(() => {
    ...
    return orders.filter((o) => {
      if (o.status !== "En cours de livraison" && o.status !== "Partiellement livrée") return false;
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      netBenefit, ordersCount, topClients,
      netMarginPercentage, brutMarginPercentage, expensesPercentage, averageOrderValue
    };
  ...
  const handleWalletTransaction = async (customerId, amountChange, orderPaymentDelta, note, currentPayments, setPayments) => {
    try {
      ...
      showToast(`Portefeuille mis à jour ! (${amountChange > 0 ? '+' : ''}${amountChange} DA)`);
    } catch(e) {
      showToast("Erreur lors de la mise à jour du portefeuille.", "error");
    }
  }
  ...
  const handleAiAnalysis = async () => {
    ...
    try {
      setTimeout(() => {
        setAiAnalysis(
          `Analyse du mois : Ton panier moyen est de ${parseFloat(stats.totalRevenue / stats.ordersCount || 0).toFixed(2)} DA, c'est super ! 1. Tente de faire de l'upsell (proposer un accessoire) pour augmenter ce panier. 2. Tes frais publicitaires représentent ${parseFloat((stats.totalSponsors / stats.totalRevenue) * 100 || 0).toFixed(2)}% du CA, surveille ce ratio. 3. Relance tes clientes fidèles avec un petit code promo pour écouler le stock restant.`
        );
        setAiLoading(false);
      }, 2000);
    } catch (err) {
  ...
  return (
    <div className="flex h-screen overflow-hidden font-sans bg-[#FDFBF7] text-[#4A3F35] print:block print:h-auto print:overflow-visible print:bg-white">
      ...
      <datalist id="sizeList">
        {config.sizes?.map((s) => <option key={s} value={s} />)}
      </datalist>
      <datalist id="colorList">
        {config.colors?.map((c) => <option key={c} value={c} />)}
      </datalist>
      ...
      <aside className="hidden md:flex w-64 border-r border-[#E8D5C4]/30 p-6 flex-col gap-6 bg-white/50 backdrop-blur-md z-10 shadow-sm print:hidden">
        ...
        <button onClick={() => signOut(auth)} className="w-full mt-2 py-2 text-[10px] uppercase font-bold text-gray-400 hover:text-red-400 transition-all">
          Déconnexion
        </button>
      </aside>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-8">
              ...
              <div className="bg-[#FAF7F2]/80 p-4 md:p-6 rounded-2xl md:rounded-[2rem] shadow-sm border border-[#E8D5C4]/30 flex flex-col relative">
                <div className="flex justify-between items-center mb-4 relative z-10">
                  ...
                  <button onClick={handleSaveNote} className="px-3 py-1.5 bg-[#8D7B68] text-white rounded-lg text-[9px] font-bold uppercase shadow-sm hover:scale-105 transition-transform">
                    Sauvegarder
                  </button>
                </div>
              ...
              <table className="w-full text-left whitespace-nowrap min-w-[800px]">
                ...
                <tbody className="divide-y divide-gray-50 text-[11px]">
                  {filteredOrders.map((o) => {
                    const total = (parseFloat(o.totalVente) || 0) + (parseFloat(o.shippingNational) || 0);
                    const reste = calculateReste(o);
                    const isLate = (o.status === "En cours de livraison" || o.status === "Partiellement livrée") && lateDeliveries.find((late) => late.id === o.id);
                    ...
                    return (
                      <tr key={o.id} className={`group transition-colors ${isLate ? "bg-red-50/40 hover:bg-red-50/60" : "hover:bg-[#FAF7F2]/50"}`}>
                        ...
                        <td className="p-4">
                          <span className={`px-2 py-1 bg-white rounded-md text-[9px] uppercase font-bold shadow-sm border ${isLate ? "border-red-300 text-red-500" : "border-gray-100 text-gray-500"}`}>
                            {o.status}
                          </span>
                        </td>
                        <td className="p-4 text-right font-black text-[#8D7B68]">{formatDA(total)}</td>
                        <td className={`p-4 text-right font-black ${reste > 0 ? "text-[#EF4444]" : "text-green-500"}`}>
                        ...
                        <td className="p-4 text-right flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-all">
                          ...
                          <button
                            onClick={() => {
                              ...
                              const existingDate = o.date?.toDate ? o.date.toDate().toISOString().split("T")[0] : new Date(o.date || Date.now()).toISOString().split("T")[0];
                              setOrderPayments(o.payments || (o.advancePayment ? [{ id: Date.now(), amount: o.advancePayment, date: existingDate }] : []));
                          ...
                          <button onClick={() => setDeleteTarget({ id: o.id, collection: "orders", label: o.orderNumber })} className="text-red-300 p-1.5 hover:bg-red-50 rounded-lg">
                            <Trash2 size={16} />
                          </button>
                        </td>
                      </tr>
                  ...
                  <div key={o.id} className={`p-4 rounded-2xl shadow-sm border flex flex-col gap-3 ${isLate ? "bg-red-50/40 border-red-200" : "bg-white border-[#E8D5C4]/30"}`}>
                    <div className="flex justify-between items-start">
                      <div>
                        <span className="text-[10px] text-[#B8A99A] font-bold uppercase">
                          {o.orderNumber} {isLate && <AlertTriangle size={12} className="inline ml-1 text-red-500" />}
                        </span>
                    ...
                    <div className="flex justify-between items-center bg-[#FAF7F2]/50 p-2.5 rounded-xl border border-transparent">
                      ...
                      <div className="text-right">
                        ...
                        <p className={`font-black text-xs ${reste > 0 ? "text-red-400" : "text-green-500"}`}>{reste > 0 ? formatDA(reste) : "Payé"}</p>
                      </div>
                    </div>
                  ...
                  <div key={c.id} className="bg-white p-4 rounded-2xl shadow-sm border border-[#E8D5C4]/30 flex flex-col gap-2">
                    <div className="flex justify-between items-center">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-bold text-[#8D7B68] text-sm">{c.name}</span>
                      </div>
                    ...
                    {c.wallet > 0 && (
                      <div className="bg-green-50 px-2 py-1.5 rounded-lg flex justify-between items-center mt-1 border border-green-100">
                        <span className="text-[9px] uppercase font-bold text-green-600 flex items-center gap-1"><Wallet size={12}/> Portefeuille</span>
                        <span className="text-[10px] font-black text-green-700">{formatDA(c.wallet)}</span>
                      </div>
                    )}
                    <div className="flex justify-between items-center text-[10px] pt-2 border-t border-gray-50 text-gray-500 mt-1">
                      <span>{c.wilaya}</span>
              ...
              <table className="w-full text-xs text-left min-w-[600px]">
                <thead className="bg-[#FAF7F2]/50 border-b border-[#E8D5C4]/20 text-[#B8A99A] font-bold uppercase tracking-widest text-[9px]">
                  <tr>
                    ...
                    <th className="p-5">Portefeuille (Avoir)</th>
                      ...
                      <tr key={c.id} className="group hover:bg-[#FAF7F2]/50">
                        ...
                        <td className="p-5 space-y-1">
                          <div className="flex items-center gap-2 text-[#4A3F35] font-medium">
                            <Phone size={12} className="text-[#D4B996]" /> {c.phone}
                          </div>
                        </td>
                        <td className="p-5">
                          {c.wallet > 0 ? (
                            <span className="bg-green-100 text-green-700 font-bold px-2.5 py-1 rounded-lg flex items-center gap-1.5 w-fit border border-green-200">
                              <Wallet size={12}/> {formatDA(c.wallet)}
                            </span>
                          ) : (
                            <span className="text-gray-300 italic">0.00 DA</span>
                          )}
                        </td>
                        <td className="p-5 space-y-1">
                          <div className="flex items-center gap-2 text-[#4A3F35] font-medium">
          ...
          <div className="space-y-4 animate-in slide-in-from-bottom-4">
            <div className="bg-[#FAF7F2] p-4 rounded-2xl md:rounded-[2rem] border border-[#E8D5C4]/30 shadow-sm flex justify-between items-center mb-4">
              <span className="text-[10px] md:text-xs uppercase font-bold text-[#B8A99A] tracking-widest">
                Poids Total Facturé {filterMonth !== 0 && `(Mois ${filterMonth})`}
              </span>
                        ...
                        .map((s) => (
                          <tr key={s.id}>
                            ...
                            <td className="p-3 text-right">
                              <button onClick={() => setDeleteTarget({ id: s.id, collection: "sponsors", label: `Sponsor` })} className="text-red-300 md:text-red-200 md:hover:text-red-400"><Trash2 size={14} /></button>
                            </td>
                          </tr>
                        ))}
                    </tbody>
                  ...
                  {expenses
                    ...
                    .map((e) => (
                      <div key={e.id} className="flex justify-between p-3 md:p-4 bg-red-50/30 rounded-xl border border-red-50">
                        <div>
                          <p className="text-[10px] font-black uppercase text-red-400">{e.label}</p>
                          <p className="text-[8px] text-gray-400 font-bold">{e.date}</p>
                        </div>
                    ...
                    .map((r) => (
                      <div key={r.id} className="flex justify-between items-center p-3 bg-[#FAF7F2]/50 rounded-xl border border-[#E8D5C4]/20">
                        ...
                        <div className="flex items-center gap-3 md:gap-4 font-black text-[#8D7B68] text-sm">
                          {parseFloat(r.rate).toFixed(2)} DA <button onClick={() => setDeleteTarget({ id: r.id, collection: "currencyRates", label: `Taux` })} className="text-red-300 md:text-red-200"><Trash2 size={14} /></button>
                        </div>
                      </div>
                    ))}
                </div>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 w-full bg-white/95 backdrop-blur-xl border-t border-[#E8D5C4]/50 flex justify-around items-center px-2 pt-2 pb-[max(16px,env(safe-area-inset-bottom))] z-[900] shadow-[0_-10px_40px_rgba(141,123,104,0.1)] print:hidden">
        <button onClick={() => setActiveTab("dashboard")} className={`flex flex-col items-center gap-1 p-2 transition-colors ${activeTab === "dashboard" ? "text-[#8D7B68]" : "text-[#B8A99A]"}`}>
          <LayoutDashboard size={20} />
          <span className="text-[8px] font-bold">Bord</span>
        </button>
        <button onClick={() => setActiveTab("orders")} className={`relative flex flex-col items-center gap-1 p-2 transition-colors ${activeTab === "orders" ? "text-[#8D7B68]" : "text-[#B8A99A]"}`}>
          <Package size={20} />
          {lateDeliveries.length > 0 && <span className="absolute top-1 right-2 w-2.5 h-2.5 bg-red-500 rounded-full border-2 border-white"></span>}
          <span className="text-[8px] font-bold">Ventes</span>
        </button>
        <button onClick={() => setActiveTab("customers")} className={`flex flex-col items-center gap-1 p-2 transition-colors ${activeTab === "customers" ? "text-[#8D7B68]" : "text-[#B8A99A]"}`}>
          <Users size={20} />
          <span className="text-[8px] font-bold">Clientes</span>
        </button>
        <button onClick={() => setActiveTab("arrivages")} className={`flex flex-col items-center gap-1 p-2 transition-colors ${activeTab === "arrivages" ? "text-[#8D7B68]" : "text-[#B8A99A]"}`}>
          <Globe size={20} />
...
{/* --- MODALS --- */}
      ...
      {showAddArrivage && (
        <ArrivageModal editingArrivage={editingArrivage} arrivageNumber={arrivageNumber} setArrivageNumber={setArrivageNumber} arrivageDate={arrivageDate} setArrivageDate={setArrivageDate} handleSaveArrivage={handleSaveArrivage} onClose={() => { setShowAddArrivage(false); setEditingArrivage(null); }} />
      )}
      {showCostBreakdown && (
        <CostBreakdownModal order={showCostBreakdown} onClose={() => setShowCostBreakdown(null)} formatDA={formatDA} calculateTotals={calculateTotals} />
      )}
      {showReceipt && (
        <ReceiptModal order={showReceipt} onClose={() => setShowReceipt(null)} formatDA={formatDA} />
      )}
      {showDeliverySlip && (
        <DeliverySlipModal order={showDeliverySlip} customers={customers} onClose={() => setShowDeliverySlip(null)} formatDA={formatDA} />
      )}
      {showCustomerHistory && (
      ...
      {showCalculator && (
        <PriceCalculatorModal onClose={() => setShowCalculator(false)} currencyRates={currencyRates} formatDA={formatDA} />
      )}
          ...
          <div className="bg-white w-full md:max-w-xs rounded-t-[2rem] md:rounded-[2rem] p-6 pb-10 md:pb-6 shadow-2xl animate-in slide-in-from-bottom-4 md:zoom-in-95">
            ...
            <form
              onSubmit={async (e) => {
                e.preventDefault();
                const fd = new FormData(e.target);
                await addDoc(collection(db, "artifacts", appId, "public", "data", "currencyRates"), {
      ...
      {showConfig && (
        <ConfigModal config={config} saveConfig={saveConfig} setConfig={setConfig} onClose={() => setShowConfig(false)} />
      )}
      {deleteTarget && (
        <DeleteModal deleteTarget={deleteTarget} performDelete={performDelete} onClose={() => setDeleteTarget(null)} />
      )}
    </div>
  );
};
...
export default function App() {
  ...
  if (loading)
    return (
      <div style={{ height: "100vh", display: "flex", justifyContent: "center", alignItems: "center", backgroundColor: "#FDFBF7", color: "#8D7B68", fontFamily: "serif" }}>
        Chargement...
      </div>
    );
...
const OrderModal = ({
  ...
  const customerWallet = parseFloat(activeCustomer?.wallet || 0);
  ...
  const sortedWilayas = Object.keys(customers.reduce((acc, c) => {
  ...
  const montantEnAlgerie = orderItems
    .filter((item) => item.status === "Reçu" || item.status === "Livré" || parseFloat(item.weightG) > 0)
                  ...
                  {sortedWilayas.map((w) => (
                    <optgroup key={w} label={w} className="bg-gray-50">
                      {groupedCustomers[w].map((c) => (
                        <option key={c.id} value={c.id} className="text-[#4A3F35]">
                          {c.name} • {c.deliveryMode === "stopdesk" ? "Stopdesk" : "Dom"}
                        </option>
                      ))}
                    </optgroup>
                  ))}
              ...
              <div className="space-y-1">
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    setOrderItems(orderItems.map((item) => ({ ...item, arrivageId: val })));
                  }}
            ...
            <div className="flex flex-col shrink-0">
              ...
              <div className="space-y-3 pr-1 pb-2">
                {orderItems.map((item, index) => {
                  const isRefunded = item.status === "Remboursé (Fourn.)";
                  const isCanceled = item.status === "Annulé";
                  return (
                    <div key={item.id} className={`bg-white border ${isRefunded || isCanceled ? 'border-red-200 bg-red-50/20 opacity-80' : 'border-[#E8D5C4]/60'} p-4 md:p-3 rounded-2xl shadow-sm flex flex-col gap-3 md:gap-2 relative pl-8 pt-6 md:pt-3 shrink-0`}>
                      ...
                      <div className="grid grid-cols-2 md:flex flex-wrap md:flex-nowrap gap-2 items-center bg-gray-50/50 p-2 rounded-xl border border-gray-100">
                        ...
                        <select className="col-span-2 md:col-span-1 w-full md:w-auto md:flex-1 p-2 md:p-1.5 rounded-lg bg-white border border-gray-100 outline-none text-[11px] md:text-[10px] font-bold text-[#8D7B68] shadow-sm" value={item.status || "En attente"} onChange={(e) => setOrderItems(orderItems.map((oi) => oi.id === item.id ? { ...oi, status: e.target.value } : oi))}>
                          ...
                          <option value="Remboursé (Fourn.)">Remboursé (Fournisseur)</option>
                          <option value="Annulé">Annulé (Client)</option>
                        </select>
                        <div className="col-span-1 flex items-center gap-1 bg-white px-2 py-2 md:py-1.5 rounded-lg shadow-sm border border-gray-100 md:w-24">
                          ...
                          <input
                            ...
                            className={`w-full outline-none text-xs font-bold text-center md:text-right bg-transparent ${isCanceled ? 'opacity-30 line-through' : ''}`}
                            value={item.weightG}
                            onChange={(e) => {
                              const val = e.target.value;
                        ...
                        <div className="col-span-1 flex items-center gap-1 bg-white px-2 py-2 md:py-1.5 rounded-lg shadow-sm border border-gray-100 md:w-24">
                          ...
                          <input type="number" step="0.01" placeholder="€" className={`w-full outline-none text-xs font-bold text-center md:text-right bg-transparent ${isRefunded || isCanceled ? 'opacity-30 line-through text-red-500' : ''}`} value={item.priceAchatEuro} onChange={(e) => setOrderItems(orderItems.map((oi) => oi.id === item.id ? { ...oi, priceAchatEuro: e.target.value } : oi))} />
                        </div>
                        <div className="col-span-2 flex items-center gap-2 mt-1 md:mt-0">
                          <div className={`flex items-center gap-1 px-3 py-2.5 md:py-1.5 rounded-lg shadow-sm border flex-1 ${isRefunded || isCanceled ? 'bg-red-50 border-red-200' : 'bg-[#F3E8E2]/40 border-[#E8D5C4]/60'}`}>
                            <span className="text-[9px] font-bold text-[#D4B996] uppercase">Vente DA</span>
                            <input type="number" className={`w-full outline-none text-sm md:text-xs font-black text-right bg-transparent ${isRefunded || isCanceled ? 'opacity-30 line-through text-red-500' : 'text-[#8D7B68]'}`} value={item.priceVente} onChange={(e) => setOrderItems(orderItems.map((oi) => oi.id === item.id ? { ...oi, priceVente: e.target.value } : oi))} />
                          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 shrink-0">
              <div className="bg-white p-4 md:p-5 rounded-2xl md:rounded-[1.5rem] border border-[#E8D5C4]/40 flex flex-col h-full shadow-sm">
                ...
                {activeCustomer && (
                  <div className="mb-4 bg-green-50/50 p-3 rounded-xl border border-green-100 flex flex-col gap-2">
                    <div className="flex justify-between items-center">
                      <span className="text-[10px] uppercase font-bold text-green-600 flex items-center gap-1.5"><Wallet size={12} /> Portefeuille Cliente</span>
                      <span className="text-xs font-black text-green-700">{formatDA(customerWallet)}</span>
                    </div>
                    {customerWallet > 0 && resteToPay > 0 && (
                      <button
                        type="button"
                        onClick={() => {
                          const amountToUse = Math.min(customerWallet, resteToPay);
                          handleWalletTransaction(activeCustomer.id, -amountToUse, amountToUse, "Payé via Portefeuille", orderPayments, setOrderPayments);
                        }}
                        className="w-full py-2 bg-green-500 text-white rounded-lg text-[10px] font-bold uppercase shadow-sm hover:bg-green-600 transition-colors"
                      >
                        Utiliser {formatDA(Math.min(customerWallet, resteToPay))} du portefeuille
                      </button>
                    )}
                  </div>
                )}
                ...
                <div className="flex-1 overflow-y-auto space-y-2 mb-3 min-h-[80px] custom-scrollbar">
                  {orderPayments.length === 0 ? (
                    ...
                    orderPayments.map((p) => (
                      <div key={p.id} className="flex justify-between items-center bg-gray-50 p-2 rounded-lg border border-gray-100">
                        <div>
                          <p className="text-[10px] font-bold text-gray-500">{p.date}</p>
                          {p.note && <p className="text-[8px] text-[#D4B996] font-bold uppercase">{p.note}</p>}
                        </div>
                        <div className="flex items-center gap-3">
                          <span className={`text-xs font-black ${p.amount < 0 ? 'text-red-500' : 'text-green-600'}`}>{formatDA(p.amount)}</span>
                ...
                <div className="flex gap-2 bg-gray-50 p-2 rounded-xl border border-gray-100">
                  ...
                  <button type="button" onClick={() => {
                      if (newPaymentAmount) {
                        setOrderPayments([...orderPayments, { id: Date.now(), amount: parseFloat(newPaymentAmount), date: newPaymentDate }]);
                        setNewPaymentAmount("");
                      }
                    }} className="bg-green-500 text-white p-2 rounded-lg shadow-sm"><Plus size={16} />
                  </button>
                </div>
              </div>
              ...
              <div className="bg-[#FAF7F2]/60 p-4 md:p-5 rounded-2xl md:rounded-[1.5rem] border border-[#E8D5C4]/40 flex flex-col justify-between">
                ...
                <div className="space-y-2 mt-4">
                  ...
                  {overpayment > 0 && activeCustomer && (
                    <div className="bg-red-50 p-3 rounded-xl border border-red-100 mt-2 mb-2">
                      <p className="text-[10px] text-red-500 font-bold uppercase mb-2">⚠️ Trop-perçu de {formatDA(overpayment)}</p>
                      <button
                        type="button"
                        onClick={() => handleWalletTransaction(activeCustomer.id, overpayment, -overpayment, "Transfert vers Portefeuille", orderPayments, setOrderPayments)}
                        className="w-full py-2 bg-white text-red-500 border border-red-200 rounded-lg text-[9px] font-bold uppercase shadow-sm hover:bg-red-50 transition-colors"
                      >
                        Transférer {formatDA(overpayment)} au portefeuille
                      </button>
                    </div>
                  )}
                  ...
                  <div className="flex justify-between items-center pt-3 border-t border-[#E8D5C4]/40 mt-2">
                    ...
                    <span className={`text-2xl font-serif font-bold ${resteToPay > 0 ? "text-[#8D7B68]" : "text-green-500"}`}>
                      {formatDA(resteToPay)}
                    </span>
                  </div>
    ...
    <div className="bg-white w-full md:max-w-md rounded-t-[2rem] md:rounded-[2.5rem] p-6 md:p-8 shadow-2xl animate-in slide-in-from-bottom-4">
      ...
      <form onSubmit={handleSaveArrivage} className="space-y-4 pb-4 md:pb-0">
        ...
        <div className="space-y-1">
          ...
          <select name="country" className="w-full p-3.5 rounded-xl bg-gray-50 text-sm font-bold text-[#8D7B68] outline-none">
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      if (val && !(config[s.k] || []).includes(val)) {
                        const nc = { ...config, [s.k]: [...(config[s.k] || []), val] };
            ...
            <table className="w-full text-xs text-left min-w-[600px]">
              ...
              <tbody className="divide-y divide-gray-50 text-[11px]">
                {processed.map((item, idx) => {
                  ...
                  const isCanceled = item.status === "Annulé";
                  return (
                    <tr key={idx} className={`hover:bg-[#FAF7F2]/30 ${isRefunded || isCanceled ? 'opacity-50' : ''}`}>
                      <td className="p-4 font-medium text-[#4A3F35] max-w-[150px] truncate">
                        {item.name || "Article"}
                        {isRefunded && <span className="ml-2 text-[8px] text-red-500 font-bold uppercase block">Remboursé (Fourn.)</span>}
                        {isCanceled && <span className="ml-2 text-[8px] text-red-500 font-bold uppercase block">Annulé</span>}
                      </td>
...
const DeliverySlipModal = ({ order, customers, onClose, formatDA }) => {
  ...
  const totalWeight = displayedItems.reduce((sum, item) => sum + (parseFloat(item.weightG) || 0), 0) / 1000;
  const itemCount = displayedItems.length;
  const subtotal = displayedItems.reduce((sum, i) => sum + (parseFloat(i.priceVente) || 0), 0);
      ...
      <div className="bg-white w-full max-w-md rounded-[2rem] shadow-2xl relative animate-in zoom-in-95 flex flex-col max-h-[90vh] overflow-hidden print:w-full print:max-w-full print:max-h-none print:shadow-none print:rounded-none print:animate-none print:transform-none print:border-none print:h-auto print:overflow-visible">
        ...
        <div className="absolute top-4 left-4 right-4 flex justify-between z-20 print:hidden">
          <div className="flex gap-2 bg-gray-50/90 p-1 rounded-full backdrop-blur-md shadow-sm border border-gray-200">
            ...
            <button onClick={() => setIsPartial(true)} className={`px-3 py-1.5 rounded-full text-[10px] font-bold transition-all flex items-center gap-1 ${isPartial ? 'bg-[#8D7B68] text-white shadow-sm' : 'text-gray-400 hover:text-[#8D7B68]'}`}>
              Reçus 
              {allItems.length - displayedItems.length > 0 && isPartial && <span className="bg-white text-[#8D7B68] px-1.5 rounded-full text-[8px]">{displayedItems.length}/{allItems.length}</span>}
            </button>
          </div>
          <div className="flex gap-2">
        ...
        <div className="overflow-y-auto custom-scrollbar flex-1 pb-8 print:overflow-visible print:pb-0 print:h-auto">
          <div className="bg-[#FAF7F2] p-8 pt-16 md:pt-14 border-b border-[#E8D5C4]/40 text-center relative print:bg-white print:border-b-2 print:border-black print:p-4">
            ...
            <h2 className="font-serif text-2xl font-bold text-[#8D7B68] tracking-widest mb-1 print:text-black">
              BORDEREAU {isPartial && <span className="text-sm block mt-1 font-sans tracking-normal opacity-80">(Livraison Partielle)</span>}
            </h2>
          ...
          <div className="p-8 space-y-6 print:p-4 print:space-y-4">
              ...
              <div className="space-y-1.5 text-sm">
                ...
                <div className="bg-gray-50 p-3 rounded-xl mt-2 text-xs text-gray-600 border border-gray-100 print:bg-white print:border print:border-black print:text-black print:p-2">
                  ...
                  <p className="flex items-center gap-1.5 font-bold">
                    <MapPin size={12} className="text-[#D4B996] print:text-black" /> {customer?.deliveryMode === "stopdesk" ? `Stopdesk: ${customer?.stopdeskName}` : "Livraison à Domicile"}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div>
              <h4 className="text-[9px] uppercase tracking-widest text-gray-400 font-bold border-b border-gray-100 pb-2 mb-3 print:text-gray-800 print:border-black">
                Détails du colis {isPartial && <span className="text-orange-400 normal-case tracking-normal ml-2 bg-orange-50 px-2 py-0.5 rounded-md border border-orange-100 print:hidden">Il reste {allItems.length - displayedItems.length} article(s) en attente</span>}
              </h4>
              <div className="flex gap-4 mb-4">
                <div className="flex-1 bg-[#FAF7F2]/50 p-3 rounded-xl border border-[#E8D5C4]/30 text-center print:bg-white print:border print:border-black print:p-2">
                  <p className="text-[10px] text-[#B8A99A] uppercase font-bold mb-1 print:text-gray-800">Articles</p>
              ...
              <div className="space-y-2 bg-gray-50 p-3 rounded-xl border border-gray-100 print:bg-white print:border print:border-black print:p-2">
                {displayedItems.length === 0 ? (
                  <p className="text-xs text-center text-gray-400 py-4 font-bold italic">Aucun article "Reçu" pour cette commande.</p>
                ) : (
                  displayedItems.map((item, idx) => (
                    <div key={idx} className="flex flex-col border-b border-gray-200/50 last:border-0 pb-2 last:pb-0 print:border-gray-300">
                      <div className="flex justify-between items-start text-xs">
                        <span className="text-[#4A3F35] font-bold pr-2 print:text-black">• {item.name || "Article"}</span>
                        <span className="text-gray-500 text-[10px] whitespace-nowrap font-bold print:text-black">{item.size} / {item.color}</span>
                      </div>
                      <span className="text-[10px] text-[#B8A99A] ml-2.5 mt-0.5 font-bold uppercase tracking-wider print:text-gray-600">{item.category || "Catégorie non spécifiée"}</span>
                    </div>
                  ))
                )}
              </div>
            ...
            <div className="bg-[#8D7B68] p-5 rounded-2xl text-white text-center shadow-md relative overflow-hidden print:bg-white print:border-2 print:border-black print:text-black print:shadow-none print:p-3">
              <p className="text-[10px] uppercase tracking-widest font-bold mb-1 print:text-gray-800">Montant à encaisser à la livraison</p>
        ...
        <div className="p-8 md:p-10 pt-12 md:pt-12 text-center overflow-y-auto custom-scrollbar flex-1 print:overflow-visible print:p-4 print:h-auto print:pt-4">
          ...
          <div className="bg-white p-4 md:p-5 rounded-2xl border border-[#E8D5C4]/30 text-left mb-6 md:mb-8 relative overflow-hidden shadow-sm print:bg-white print:border print:border-black print:shadow-none print:p-4">
            ...
            <div className="space-y-3 print:space-y-2">
              {items.map((item, idx) => (
                <div key={idx} className="flex flex-col border-b border-gray-100 last:border-0 pb-2 last:pb-0 print:border-gray-300">
                  <div className="flex justify-between items-start text-xs">
                    <span className={`font-bold pr-2 print:text-black ${item.status === 'Remboursé (Fourn.)' || item.status === 'Annulé' ? 'line-through text-gray-400' : 'text-[#4A3F35]'}`}>
                      {item.name || "Article"}
                    </span>
                    <span className={`font-black print:text-black whitespace-nowrap ${item.status === 'Remboursé (Fourn.)' || item.status === 'Annulé' ? 'line-through text-gray-400' : 'text-[#8D7B68]'}`}>
          ...
          <div className="space-y-2 md:space-y-3 text-xs bg-white/50 p-4 md:p-5 rounded-2xl border border-[#E8D5C4]/20 print:bg-transparent print:border-none print:p-0">
            <div className="flex justify-between text-[#8D7B68]/70 font-bold print:text-black print:font-medium">
              <span>Sous-total</span><span>{formatDA(subtotal)}</span>
            </div>
            <div className="flex justify-between text-[#8D7B68]/70 font-bold print:text-black print:font-medium">
              <span>Livraison</span><span>{formatDA(shipping)}</span>
            </div>
            <div className="flex justify-between text-green-600/80 font-bold print:text-black">
              <span>Avance</span><span>- {formatDA(advance)}</span>
            </div>
            ...
            <div className="flex justify-between font-serif font-bold text-lg md:text-xl text-[#8D7B68] print:text-black">
              <span>Reste à payer</span><span>{total > 0 ? formatDA(total) : "0.00 DA"}</span>
            </div>
          </div>
